...
static int dm9000c_probe ( struct platform_device* pdev )
{
	...
	{
		irq = res->start;
		printk ( "get dm9000c irq:%d",irq );
	}
	else
	{
		printk ( "get dm9000c irq error" );
		return -1;
	}
	return dm9000c_init();
...
static int dm9000c_remove ( struct platform_device* pdev )
{
	dm9000c_exit();
	return 0;
...
static const struct of_device_id of_match_dm9000c[] =
{
	{.compatible = "davicom,dm9000", .data= NULL}, //匹配名字为davicom,dm9000的子节点
	{/* sentinel */},

};


static struct platform_driver dm9000c_drv =
{
	.probe = dm9000c_probe,
	.remove = dm9000c_remove,
	.driver ={
		.name = "dm9000c",
		.of_match_table = of_match_dm9000c, //表示匹配哪些设备树节点，匹配成功后调用probe函数
	}
};

static int dm9000c_drv_init ( void )
{
	platform_driver_register ( &dm9000c_drv );
	return 0;
}

static void dm9000c_drv_exit ( void )
{
	platform_driver_unregister ( &dm9000c_drv );
}

module_init ( dm9000c_drv_init );
module_exit ( dm9000c_drv_exit );
MODULE_LICENSE ( "GPL" );
